...
export default function EditBroadcastPage() {
  ...
  useEffect(() => {
    ...
    if (s) {
      ...
      setBgmStart(s.bgmStartSeconds != null ? String(s.bgmStartSeconds) : "");
      setBgmEnd(s.bgmEndSeconds != null ? String(s.bgmEndSeconds) : "");
      ...
      if (s.voice) {
        const preset = GOOGLE_TTS_PRESETS.find((p) => p.voice === s.voice);
        if (preset) {
          setGooglePresetId(preset.id);
        }
      }
    }
    setLoaded(true);
    ...
    try {
      ...
      if (hasBgm && ytPlayer.ready) {
        if (musicMode === "background") {
          // 배경음악: 음성과 동시에 시작, 음성이 끝나면 반복 여부에 따라 다시 세트 실행
          ...
          // 중간음악: 음성 끝난 뒤 한 번 실행, onBgmEndRef에서 반복 처리
      ...
      // 재생 시점 기록: 마지막 방송일시 갱신
      const now = new Date().toISOString();
      const base = sessionBase;
      if (base) {
        const updated: Session = { ...base, lastPlayedAt: now, updatedAt: now };
        saveSession(updated, itemsRef.current, eventItemsRef.current);
        setSessionBase(updated);
      }
    } catch {
      setGenerateError("재생을 시작할 수 없습니다.");
    }
  }, [sessionId, hasBgm, ytPlayer, bgmVolume, repeatEnabled, musicMode, sessionBase]);
  ...
  useEffect(() => {
    // 중간음악 모드일 때: YouTube 구간이 끝난 뒤 동작 정의 (Edit 페이지도 새 방송과 동일 동작)
    onBgmEndRef.current = () => {
      if (musicMode !== "interval") return;
      if (!repeatEnabled) {
        setIsPlaying(false);
        return;
      }
  ...
  if (!sessionBase) {
    return (
      <main className="min-h-screen bg-[var(--bg)] p-8">
        <p className="text-stone-500">해당 방송을 찾을 수 없습니다.</p>
        <Link href="/" className="mt-2 inline-block text-amber-600 hover:underline">
          ← 첫 화면
        </Link>
      </main>
    ...
    <main className="min-h-screen bg-[var(--bg)]">
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <p className="mt-1 text-sm text-stone-500">
          기존에 저장된 방송 내용을 수정하고, 필요하다면 배경 음악 설정을 바꾼 뒤 다시 음성 파일을 생성해 재생할 수 있습니다.
        </p>
