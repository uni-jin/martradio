...
export default function NewBroadcastPage() {
  ...
  const updateItem = (target: "super" | "event") => (id: string, field: string, value: string | number | boolean | null) => {
    const updater = (prev: typeof items) =>
  ...
  const removeItem = (target: "super" | "event") => (id: string) => {
    if (target === "super") setItems((prev) => prev.filter((it) => it.id !== id));
    else setEventItems((prev) => prev.filter((it) => it.id !== id));
    setScriptTouched(false);
  };
  ...
  const handleSave = () => {
    ...
    setSaved(true);
    router.push(`/broadcast/${sessionId}/play`);
  };
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <section className="mt-8 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-4">
            ...
            {/* 반복 간격은 재생 화면에서 설정·활용 예정 (세션에는 기본값 저장) */}
          </div>
        </section>
...
<th className="w-28 py-2 text-left font-medium text-stone-600">정상가(선택)</th>
        ...
        <section className="mt-8 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-xs text-stone-400">
            선택된 상품만 포함됩니다. 필요하면 문구를 직접 수정할 수 있습니다.
          </p>
        </section>
        ...
        <section className="mt-8 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4">
            ...
            {saved && (
              <span className="text-sm text-green-600">저장되었습니다. 첫 화면에서 불러올 수 있습니다.</span>
            )}
          </div>
          </section>
      </div>
